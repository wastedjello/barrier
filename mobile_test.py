...
# MobileNet-SSD model files (Caffe)
MODEL_PROTOTXT = "MobileNetSSD_deploy.prototxt"
MODEL_WEIGHTS = "MobileNetSSD_deploy.caffemodel"
# Minimum detection confidence for SSD
SSD_CONF_THRESH = 0.45
# ==========================================
...
if os.path.exists(MODEL_PROTOTXT) and os.path.exists(MODEL_WEIGHTS):
    net = cv2.dnn.readNetFromCaffe(MODEL_PROTOTXT, MODEL_WEIGHTS)
    # Optional acceleration:
    # net.setPreferableBackend(cv2.dnn.DNN_BACKEND_DEFAULT)
    # net.setPreferableTarget(cv2.dnn.DNN_TARGET_CPU)
    print("ℹ️ MobileNet-SSD loaded")
else:
    net = None
    print("⚠️ MobileNet-SSD model files not found. Set MODEL_PROTOTXT and MODEL_WEIGHTS paths.")

# ---------- Detection function (SSD-based) ----------
def detect_plate_and_ocr(frame):
    """
    Uses MobileNet-SSD to propose vehicle regions, crops each detection,
    preprocesses and runs Tesseract OCR with confidence.
    Returns: (text, bbox (x,y,w,h), roi_path, conf)
    """
    if net is None:
        # fallback to contour-based method if SSD missing
        # (simple contour approach similar to original)
    ...
    for i in range(detections.shape[2]):
        ...
        if score < SSD_CONF_THRESH:
            continue
        class_id = int(detections[0, 0, i, 1])
        label = CLASSES[class_id] if class_id < len(CLASSES) else str(class_id)

        # Focus on vehicle classes likely to contain plates
        if label not in ("car", "bus", "motorbike", "train", "truck"):
            continue
        ...
        score_metric = conf * score
        if best_candidate is None or score_metric > best_candidate["metric"]:
            best_candidate = {
                "text": text,
                "bbox": (x0, y0, bw, bh),
                "roi": roi_fname,
    ...
    if best_candidate:
        if valid_plate(best_candidate["text"]):
            return best_candidate["text"], best_candidate["bbox"], best_candidate["roi"], best_candidate["conf"]
        else:
            try:
                bx, by, bw, bh = best_candidate["bbox"]
                failed_fname = f"{FAILED_ROI_DIR}/failed_{int(time.time())}_{bx}_{by}.jpg"
                cv2.imwrite(failed_fname, frame[by:by+bh, bx:bx+bw])
            except Exception:
                pass
            return None, None, None, 0.0
